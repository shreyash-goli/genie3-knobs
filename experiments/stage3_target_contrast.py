"""Stage 3 -- two-target contrast: does the learned policy act target-dependently?

Targets (Section 2.2, Stage 3 / config.py):
  * BHRF1  (01_bhrf1)    -- longer binders demonstrably helped recover missed hotspots.
  * InsulinR (06_insulinr)-- length did NOT generalise (B59/B91 resisted interventions).

The actual point of a *learned* policy (vs. one fixed global heuristic) is target-dependent
action selection.  This script runs the full-lever comparison on just these two targets and
then inspects each learned policy's per-target greedy action -- specifically whether it
extends binder length on BHRF1 but not on InsulinR.  It prints a clear yes/no on
target-dependent behaviour and writes the per-target policy tables to the stage summary.
"""
...
def main() -> int:
    ...
    print("\n=== Stage 3: target-dependent behaviour check ===")
